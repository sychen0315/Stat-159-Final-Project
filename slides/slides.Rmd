...
- Select the following columns: 
    - Number of undergraduate student (UGDS)
    - admission rate (ADM_RATE)
    - average cost of attendence, tuition and fees (COSTT4_A)
    - Mean and median earnings (MD_EARN_WNE_P10)
    - completion rates within 4 years (C100_4)
    - percent of undergraduates receiving federal loans (PCTFLOAN)
    - minority ratio (calculated by the sum of percentages of different minorty groups)
    - students applied (calculated by dividing number of undergrads by admission rate)
- Remove rows that contain at least one null values

## Categorization:

- Cluster schools by regions: west, midwest, northeat and south and then by whether the schools is located in a major city or countryside
...
- Run simulation to split the data into 3: 1 train and test set
...
print(xtable(summary(ols_full_fit), digits = c(0,3,3,3,3)), comment = FALSE, type = "HTML")
```
...
- Cluster MN regression result:

- 
```{r results= 'asis', echo = FALSE}
load("../data/regression-data/MN-ols-model.RData")
print(xtable(summary(MN_model)$coefficients), comment = FALSE, type = "HTML")
```



- Significant coefficients for all variables (lowest average number of students applied)
- Significant coefficient for minority ratio (Low minority ratio)
...
- Midwest and South Region
 ...
 - Second most important factor for Midwest region is minority ratio, so should focus on promote diversity
    - Offer student loans to minority students to make sure they have access to financial resources
    - Adjust admission process accordingly
    - Encourage minority student groups on campus
 - Second most important factor for South region is graduation rate, so should focus on improve completiton rate
    - Aim to offer personalized experience for each student by reducing class size
    - Expand academic advising program to address people's academic concerns in time and reach out to people who have unsatisfactory academic record
